...
can be used to test the LDAP integration of @webScheduler.
...
## Enabling Integration in @webScheduler

After making sure that the local OpenLDAP server works, @webScheduler will be able to connect to it.

For this you will need to go to the Backend > Settings > Integrations > LDAP and enable the integration from there. 

This settings page also requires some configuration for @webScheduler to be able to connect to the server and pull
...
If the filter was right, you will be getting LDAP entries displayed on screen that can be imported into @webScheduler.
...
While SSO with LDAP on @webScheduler, the username will need to be matched for the SSO to work. Also, the LDAP DN 
...
If LDAP is enabled, @webScheduler will first try to check for the local credentials, but if this does not work, then 
